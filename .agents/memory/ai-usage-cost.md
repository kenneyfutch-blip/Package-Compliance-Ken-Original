...
- `aiUsageWriteHealthSnapshot()` → exposed via `GET /ai-usage/health` (dashboard:read, **uncached**, NOT org-scoped: it's process pipeline health, no tenant data). Dashboard polls it and shows an "under-reported" banner when `!healthy && failures>0`, so admins distinguish "logging is failing" from "usage genuinely dropped".
