...
- **Owner-admin must live in `ADMIN_EMAILS`, not just a DB role edit.** `provisionUser` "never downgrades an existing role," so editing a live user's `roleKey`/`role` sticks across normal logins. BUT a demo reseed (`clearDemo`) deletes the users table; on next login a runtime-provisioned owner is re-created at `DEFAULT_ROLE_KEY` (compliance_specialist) because seedDemo already created a platform_admin persona, so the bootstrap "first admin" path doesn't fire. **Symptom:** after a reset the real owner silently loses Team Dashboard / Assignments / User Management (they need `teams:read`/`users:read`, which specialist lacks) even though no permission code changed. **Fix:** put the owner's email in the `ADMIN_EMAILS` shared env var (forces platform_admin on every provision) AND update the current DB row; restart api-server to clear the ~5min AuthContext cache. **Why:** DB-only edits don't survive the next reseed; ADMIN_EMAILS does.
