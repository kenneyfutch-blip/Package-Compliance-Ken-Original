...
- **Package cards have no stored raster preview** — PDF thumbnails are rendered client-side (pdf.js first page → canvas), which downloads the full PDF per card. **Why:** it makes existing PDFs preview with zero re-upload/backfill. **How to apply:** acceptable at this app's scale; if the package list grows large, generate + store a raster preview at upload instead. Any client PDF thumbnail must cancel the render task AND `loadingTask.destroy()` on unmount or many cards leak worker/memory.
...
- **`COMPLIANCE_PUBLIC` (seed `/artwork/` dir) must be resolved by walking UP from `import.meta.url` until `artifacts/compliance/public` exists** — see `lib/assets.ts`. A fixed "up N levels" offset is wrong because the server runs from the bundled `dist/index.mjs`, which sits at a different depth than the `src/` tree it was built from; the old offset landed on `/home/runner` and seed artwork silently failed to load (5KB artwork-less exports). Object-storage (`/objects/`) uploads were unaffected, so it looked like "only some packages lose artwork".
- Teams/SharePoint "Sorry, something went wrong / File Not Found" on an uploaded proof is a SharePoint-side preview error (file uploaded but preview renderer couldn't find/render it), NOT an app bug — verified our export returns valid `application/pdf` bytes with correct `Content-Disposition`. Export filename falls back to a slug of the package name (or `pkg-<id>`) when SKU is empty, so it is never `proof--v27.pdf`.
